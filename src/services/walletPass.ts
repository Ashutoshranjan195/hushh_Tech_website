// Inline the shared wallet model logic to avoid importing server-side api/ files
// into the browser bundle (api/ files are excluded from Vite's module graph).

const DEFAULT_WALLET_ROOT_URL = "https://hushhtech.com";
const WALLET_CARD_BADGE_TEXT = "HUSHH GOLD";
const WALLET_CARD_TITLE = "Hushh Gold Investor Pass";
const WALLET_CARD_ORGANIZATION_NAME = "Hushh Technologies";
const WALLET_CARD_ORGANIZATION_FALLBACK = "Hushh";
const WALLET_CARD_STATUS = "Gold Member";

const _getTrimmed = (v: unknown) =>
  typeof v === "string" && v.trim().length > 0 ? v.trim() : "";

const _getWalletInvestmentClass = (amount: unknown) => {
...
const _buildMembershipId = (input: Record<string, unknown>) =>
  _getTrimmed(input?.slug) ||
  _getTrimmed(input?.userId) ||
  (_getTrimmed(input?.email)
    ? (_getTrimmed(input.email) as string).split("@")[0]
    : "hushh-investor");

const buildWalletCardContent = (input: Record<string, unknown> = {}) => {
  const investmentClass = _getWalletInvestmentClass(input.investmentAmount);
  const slug = _getTrimmed(input?.slug);
  const profileUrl = slug ? `${DEFAULT_WALLET_ROOT_URL}/investor/${slug}` : null;
  return {
    badgeText: WALLET_CARD_BADGE_TEXT,
    title: WALLET_CARD_TITLE,
    status: WALLET_CARD_STATUS,
    holderName: _getTrimmed(input.name) || "Hushh Investor",
    organizationName: _getTrimmed(input.organisation) || WALLET_CARD_ORGANIZATION_FALLBACK,
    investmentClass,
    investmentLabel: `Investor - ${investmentClass}`,
    membershipId: _buildMembershipId(input as Record<string, unknown>),
    email: _getTrimmed(input.email) || "—",
    profileUrl,
    passUrl: profileUrl || DEFAULT_WALLET_ROOT_URL,
  };
};

const buildSharedGoldPassPayload = (input: Record<string, unknown> = {}) => {
  ...
  return {
    ...
    barcode: { message: content.passUrl, format: "PKBarcodeFormatQR", altText: "Hushh Gold Pass QR" },
